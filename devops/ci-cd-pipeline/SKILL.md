---
name: ci-cd-pipeline
description: "Use when designing, debugging, or optimizing CI/CD pipelines (持续集成/持续部署流水线). Covers GitHub Actions, GitLab CI, Jenkins, Azure DevOps configuration, build caching, test automation, deployment strategies (blue-green/蓝绿部署, canary/金丝雀发布), and pipeline performance troubleshooting."
license: MIT
---
...
1. **分析需求** → 确定构建工具、测试框架、部署目标环境
2. **配置触发器** → 设置push/PR/tag/schedule触发条件
3. **构建阶段** → 依赖安装 + 缓存 → 编译 → 产物归档
4. **测试阶段** → 单元测试 → 集成测试 → 安全扫描（并行执行）
5. **部署阶段** → staging验证 → production发布（含健康检查和回滚策略）
6. **验证检查点** → 每阶段设置质量门禁，失败即终止

## 部署策略选择

| 策略 | 适用场景 | 回滚速度 | 风险 |
|------|----------|----------|------|
| **蓝绿部署** (Blue-Green) | 需要零停机、快速回滚 | 秒级 | 需要双倍资源 |
| **金丝雀发布** (Canary) | 大流量服务、渐进验证 | 分钟级 | 需要流量分配能力 |
| **滚动更新** (Rolling) | 无状态服务、资源有限 | 分钟级 | 短暂版本不一致 |
