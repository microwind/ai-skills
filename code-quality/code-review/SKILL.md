---
name: code-review
description: "代码审查与质量检查技能。Use when conducting code review, PR review, pull request analysis, merge request evaluation, 代码质量检查, 编码规范验证, 代码架构审查, 安全漏洞扫描, or 代码评审流程设计。"
license: MIT
---
...
1. **理解上下文**: 阅读PR描述、关联issue，确定审查重点
2. **安全审查**: 检查硬编码密钥、SQL注入、XSS、未验证输入
3. **架构审查**: 验证设计模式、模块耦合、接口设计、扩展性
4. **质量审查**: 检查函数复杂度(圈复杂度≤10)、函数长度(≤50行)、编码规范
5. **性能审查**: 识别循环拼接字符串、重复DB查询等反模式
6. **生成报告**: 按严重程度(Critical→Info)分类输出，附具体修复建议
7. **验证**: 确认所有Critical/High问题已标记，建议可操作

## 问题严重程度

| 级别 | 扣分 | 示例 |
|------|------|------|
| Critical | -20 | 硬编码密钥、SQL注入、数据泄露 |
| High | -10 | 类方法超过20个、缺少认证检查 |
| Medium | -5 | 函数超50行、循环中字符串拼接 |
| Low | -2 | 行超100字符、缺少docstring |
| Info | 0 | 风格偏好建议 |
