...
**Abstract Factory** 模式的设计目的是为一族相关对象的创建提供一致的接口
...
public class ConcretePattern implements Pattern {
    @Override
    public void execute() {
        System.out.println("Execute Abstract Factory");
    }
}
...
class ConcretePattern(Pattern):
    def execute(self):
        print("Execute Abstract Factory")
...
class ConcretePattern implements Pattern {
    execute(): void {
        console.log("Execute Abstract Factory");
    }
}
