...
聚合根是聚合的**唯一入口**，外部对象只能通过聚合根与聚合交互；它持有聚合的全局标识，负责维护聚合的所有业务不变量。

**核心规则**：
- 聚合根本身是 Entity，拥有全局唯一标识
- 外部对象只引用聚合根，不直接引用内部实体或值对象
- 聚合根负责维护所有跨子对象的业务不变量
- 仓储以聚合根为单位加载与保存（一次性加载完整聚合）
- 聚合根负责**产生（raise / record）**领域事件——把"已发生的事实"记入自身；**事件的对外发布**则由应用服务在事务边界统一完成（或经由 Outbox 投递）
- 一个事务只修改一个聚合实例
- 聚合间通过 ID 引用，不通过对象引用
...
❌ **为内部实体单独建仓储**——`OrderItemRepository`、`OrderLineRepository`
→ 一聚合一仓储；内部实体通过聚合根加载和保存

❌ **聚合根成为"上帝类"**——所有方法都堆在 Order 上，单类逾千行
→ 跨聚合的协同抽到领域服务；聚合内的复杂规则抽成规约 / 策略对象

❌ **对外暴露内部可写集合**——`getItems()` 返回可写 `List`，外部直接增删
→ 返回不可变视图（`Collections.unmodifiableList`），或只暴露查询方法（`findItemBySku()`）
