...
❌ **用领域服务替代实体行为**——`OrderService.cancel(order)` 替代 `order.cancel()`
→ 行为优先回归实体；只有跨聚合或不属于任何实体的逻辑才下沉到领域服务

❌ **领域服务直接调用基础设施**——里面注入 HTTP Client、JdbcTemplate
→ 领域服务只依赖领域接口；外部技术细节由基础设施层适配器实现

❌ **领域服务变成"杂物间 Service"**——业务逻辑、参数转换、事务、权限全往里塞
→ 领域服务只做"纯领域计算"；编排、事务、权限属于应用层
