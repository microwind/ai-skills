...
❌ **命令式命名**——`PlaceOrderEvent`、`ProcessPaymentEvent`
→ 事件是过去时事实：`OrderPlaced`、`PaymentCompleted`；命令是请求，事件是发生

❌ **事件携带可变对象引用**——把整个 `Order` 聚合对象塞进事件 payload
→ 事件应携带不可变的**数据快照**（关键字段 + 聚合 ID）；订阅方需要更多信息时按 ID 回查

❌ **业务事务内同步发布事件**——订阅者失败导致主事务回滚，跨上下文耦合死灰复燃
→ 事务内只 raise 到聚合，事务提交后由应用服务 / Outbox 发布；订阅方失败不应影响主流程
