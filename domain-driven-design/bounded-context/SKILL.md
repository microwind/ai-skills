...
**轻量 EventStorming 工作坊步骤**（推荐用于初次划分）：

```
1) 集结领域专家 + 开发 + 产品，准备一面白板与三色便签

2) 橙色便签：写下所有"已发生的业务事件"（过去时）
   例：订单已提交、库存已扣减、支付已完成、发货已创建
   不要解释、不要排序，先穷举

3) 沿时间轴把事件排成"故事线"
   争议点（同一事件多人理解不一致）= 隐藏的术语边界

4) 蓝色便签：标注触发事件的"命令"（动词）
   黄色便签:标注命令的"参与者/聚合根候选"

5) 用粗黑线圈出"语言突变"的位置——同一名词含义切换处
   每一个圈 = 一个候选限界上下文

6) 命名每个上下文 + 写出各自最重要的 3–5 个术语
   这些术语就是该上下文的通用语言种子
```

> ⚠️ **拆得过细的代价**：上下文不是越多越"DDD"。拆得过细会带来分布式事务、跨上下文调用链放大、团队认知碎片化、数据一致性维护成本陡增等问题。**判据**：当一个"业务流程"需要频繁横跨 3+ 个上下文，或频繁出现"为了保持一致性而拉一个分布式事务"时，往往是上下文划分错了——**先合并**再考虑细分。康威定律双向适用：模型边界要匹配团队边界，而团队边界本身也应稳定且可承载。
...
❌ **按数据库表划分上下文**——把表结构直接当成边界，本质仍是数据驱动设计
→ 上下文边界由业务语言与团队职责决定；数据库结构是结果，不是起点

❌ **多个上下文共享同一张表 / 同一个 ORM Entity 类**——表面上分了上下文，运行时仍是一个大泥球
→ 每个上下文有自己的模型与持久化映射；要共享数据用 API / 事件，不要共享表

❌ **追求"标准答案"上下文划分**——一次到位、永不调整
→ 上下文随业务理解加深会演化；定期复盘、允许重构边界比"一次画对"更重要
