...
工厂是 DDD 战术设计的一个构建块，源于 GoF 设计模式中的工厂模式。Eric Evans 在蓝皮书中保留这一概念的动机是：当对象或聚合的创建过程足够复杂，让对象自己"生自己"会污染领域模型时，把创建逻辑独立成工厂——**让构造即合法**。

**核心动机**：

> 将创建复杂对象和聚合的职责赋予一个独立的对象。这个对象本身不承担领域模型的业务职责，但仍是领域设计的一部分。工厂应提供一个清晰的创建接口，封装创建过程的复杂性，并确保聚合一经创建即满足其业务不变量。

> ⚠️ **工程务实**：**独立工厂类是少数场景**——大多数聚合用**构造函数 + 不变量校验**或**聚合根上的静态工厂方法**就足够。只有当创建过程需要外部依赖（仓储、策略、外部服务）或涉及多步骤组装时，才升级为独立工厂类。不要为了"看起来更 DDD"而为简单聚合包一层空壳工厂。
...
❌ **工厂只是构造函数的"套壳"**——没有任何不变量校验或业务语义
→ 工厂的价值是"构造即合法"或封装复杂创建；否则直接用构造函数

❌ **工厂放到应用层 / 基础设施层**——破坏依赖方向
→ 工厂属于领域层，与聚合同 package

❌ **抽象的方法命名**——一律 `create()` / `build()`，丢失业务意图
→ 用业务语义命名：`Subscription.fromPayment()`、`Order.placeFromCart()`、`Money.yuan(100)`
