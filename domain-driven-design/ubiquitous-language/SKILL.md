...
通用语言是 DDD 的基石。Eric Evans 在蓝皮书中将其定义为：**开发者与领域专家围绕领域模型共同构建的、严谨的（rigorous）共享语言**，贯穿于沟通、文档和代码。

**核心思想**：
- 代码中的命名 = 业务专家使用的术语
- 消除"翻译层"——开发者与业务人员讲同一种话
- 语言的变化意味着模型的变化（反之亦然）
- 双向验证：领域专家应对生硬或不足的术语提出异议；开发者应警觉歧义与不一致
- 持续演进：随着团队对业务理解加深，语言与模型同步精化
...
❌ **保留翻译层**——开发用技术词（`user_data`、`record`、`status=2`），业务用业务词（客户、保单、已批准），每次需求评审都在做翻译
→ 把业务术语直接写进代码：类名、方法名、枚举值都向业务专家对齐

❌ **同义词泛滥**——同一概念在不同模块叫 `user` / `customer` / `account` / `member`
→ 同一限界上下文内强制单一术语；不同上下文允许差异，但要在术语表中明确映射关系

❌ **术语表写完即锁柜**——项目启动时维护一次，半年后与代码漂移
→ 把术语表作为活文档，纳入代码评审：新增 / 重命名领域类时同步更新
...
**核心**：开发者和业务专家围绕领域模型共建一套严谨、共享、可演进的语言，代码直接反映业务术语。

**实践**：通过事件风暴建立术语表，代码命名使用业务概念，在限界上下文内保持语言一致；遇到生硬或歧义的用语，回到领域专家身边一起修正。
